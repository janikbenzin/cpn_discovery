...
def to_petri_net(bpmn_graph, parameters=None):
    """
    ...
    source_places = []
    sink_places = []
    for start in [node for node in bpmn_graph.get_nodes() if isinstance(node, BPMN.StartEvent)]:
        source_place = PetriNet.Place(f"source_{start.get_id()}")
        net.places.add(source_place)
        source_places.append(source_place)
    for end in [node for node in bpmn_graph.get_nodes() if isinstance(node, BPMN.EndEvent)]:
        sink_place = PetriNet.Place(f"sink_{end.get_id()}")
        net.places.add(sink_place)
        sink_places.append(sink_place)
    im = Marking()
    fm = Marking()
    for s in source_places:
        im[s] = 1
    for e in sink_places:
        fm[e] = 1
    ...
    for node in bpmn_graph.get_nodes():
        ...
        if isinstance(node, BPMN.StartEvent):
            ...
            add_arc_from_to(source_places[i], start_transition, net)
            ...
            add_arc_from_to(end_transition, sink_places[j], net)
    ...
    for flow in bpmn_graph.get_flows():
        if flow._Flow__name == "":
            ...
            source_object = trans_map[flow.get_source()][0]
            target_object = trans_map[flow.get_target()][0]
            add_arc_from_to(source_object, flow_place[flow], net)
            add_arc_from_to(flow_place[flow], target_object, net)
